#' @title Simulate an IIT Metapopulation program trajectory as a Markov Population Process.
#' 
...
#' @param \code{numReleased} is the number of insects to be released at each each release time.  This should be a matrix, with a row for each metapopulation patch and with columns equal to the number of release times.
...
#' @param \code{initStateList} a list containing numeric vectors that acts as an override allowing you to specify the initial state vector directly.
...
simulateIIT_metapopulation = function(paramsList, blockImmigrationRates, Wld_m, Wld_f_Unmated, Wld_f_Wld, stochasticInitial = TRUE, numReleased = NULL, ratioReleased = NULL, releaseMixture, contaminationProb, propTypes, releaseTimes, maxTime, initStateList = NULL, maxSize = 1000000)
{
	...
	for(p in 1:length(paramsList))
	{
		...
		if(!stochasticInitial)
		{
			...
			{
				state[paste0(names[i], "_m")] = round(propTypes[i]*Wld_m[p])
				state[paste0(names[i], "_f_Unmated")] = round(propTypes[i]*Wld_f_Unmated[p])
			...
			for(i in 1:length(names))
			{
				...
				{
					state[paste0(names[i], "_f_", names[j])] = round(propTypes[i]*Wld_f_Wld[p]*matingProbs[j])
				}
			}
		...
		{
			maleNumbers = rmultinom(1, round(Wld_m[p]), propTypes)
			unmatedFemaleNumbers = rmultinom(1, round(Wld_f_Unmated[p]), propTypes)
			friedsIndexParams = rep(NA, length(names))
			for(i in 1:length(names))
			{
			...
			matingProbs = propTypes*friedsIndexParams
			matingProbs = matingProbs/sum(matingProbs)
			matedfemaleNumbersByType = rmultinom(1, round(Wld_f_Wld[p]), propTypes)
			unmatedFemaleNumbersByType = rmultinom(1, round(Wld_f_Unmated[p]), propTypes)
			for(i in 1:length(names))
			{
		...
		if(!is.null(initStateList))
		{
			state = initStateList[[p]]
		}
		...
		for(i in 1:length(releaseTypes))
		{
			...
			numReleasedOfThisType = round(numReleased[p, 1]*releaseMixture[releaseTypes[i]])
