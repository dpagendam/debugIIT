...
suppressionObjective = 0.5*Wld_m
pastSuppressionModifier = 0.5
releaseNumbers = rep(numReleased, length(releaseDays))

simulation = list()
simulation1 = simulateIIT(params = params, Wld_m = Wld_m, Wld_f_Unmated = Wld_f_Unmated, Wld_f_Wld = Wld_f_Wld, stochasticInitial = TRUE, numReleased = rep(numReleased, length(releaseDays)), ratioReleased = NULL, releaseMixture = releaseMixture, contaminationProb = contaminationProb, propTypes = propTypes, releaseTimes = releaseDays, maxTime = maxDays, maxSize = 1000000)
...
if(length(objAchievedInd) > 0)
{
	objAchievedInd1 = objAchievedInd[1]
	objAchievedTime1 = simulation1$t[objAchievedInd1]
	objectiveState = simulation1$state[objAchievedInd1, ]
	remainingReleaseDays = c(0, releaseDays[releaseDays >= objAchievedTime1] - objAchievedTime1)
	remainingReleaseNumbers = round(pastSuppressionModifier*releaseNumbers[releaseDays >= objAchievedTime1])
	simulation2 = simulateIIT(params = params, Wld_m = Wld_m, Wld_f_Unmated = Wld_f_Unmated, Wld_f_Wld = Wld_f_Wld, stochasticInitial = TRUE, numReleased = remainingReleaseNumbers, ratioReleased = NULL, releaseMixture = releaseMixture, contaminationProb = contaminationProb, propTypes = propTypes, releaseTimes = remainingReleaseDays, maxTime = maxDays - objAchievedTime1, initState = objectiveState, maxSize = 1000000)
}

suppressionObjective = 0.1*Wld_m
pastSuppressionModifier = 0.1

malePop = simulation2$state[, "Wld_m"] + simulation2$state[, "WMel_m"]
objAchievedInd = which(malePop < suppressionObjective)
if(length(objAchievedInd1) == 0)
{
	simulation$t = c(simulation1$t[1:objAchievedInd1], simulation2$t[-1] + objAchievedTime1)
	simulation$state = rbind(simulation1$state[1:objAchievedInd1, ], simulation2$state[-1, ])
}
if(length(objAchievedInd) > 0)
{
	objAchievedInd2 = objAchievedInd[1]
	objAchievedTime2 = simulation2$t[objAchievedInd2]
	objectiveState = simulation2$state[objAchievedInd2, ]
	remainingReleaseDays = c(0, releaseDays[releaseDays >= (objAchievedTime1 + objAchievedTime2)] - (objAchievedTime1 + objAchievedTime2))
	remainingReleaseNumbers = round(pastSuppressionModifier*releaseNumbers[releaseDays >= (objAchievedTime1 + objAchievedTime2)])
	simulation3 = simulateIIT(params = params, Wld_m = Wld_m, Wld_f_Unmated = Wld_f_Unmated, Wld_f_Wld = Wld_f_Wld, stochasticInitial = TRUE, numReleased = remainingReleaseNumbers, ratioReleased = NULL, releaseMixture = releaseMixture, contaminationProb = contaminationProb, propTypes = propTypes, releaseTimes = remainingReleaseDays, maxTime = maxDays - (objAchievedTime1 + objAchievedTime2), initState = objectiveState, maxSize = 1000000)
}

simulation$t = c(simulation1$t[1:objAchievedInd1], simulation2$t[2:objAchievedInd2] + objAchievedTime1, simulation3$t[-1] + objAchievedTime1 + objAchievedTime2)
simulation$state = rbind(simulation1$state[1:objAchievedInd1, ], simulation2$state[2:objAchievedInd2, ], simulation3$state[-1, ])
